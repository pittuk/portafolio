...
export const project = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      title: 'Caso de éxito',
      type: 'object',
      description: 'Opcional. Si se completa, reemplaza la descripción plana en la página del proyecto.',
      fields: [
        defineField({ name: 'problem', title: 'Problema', type: 'text', rows: 2 }),
        defineField({ name: 'goal', title: 'Objetivo', type: 'text', rows: 2 }),
        defineField({ name: 'whatIDid', title: 'Qué hice', type: 'array', of: [{ type: 'string' }] }),
        defineField({ name: 'result', title: 'Resultado', type: 'array', of: [{ type: 'string' }] }),
      ],
    }),
